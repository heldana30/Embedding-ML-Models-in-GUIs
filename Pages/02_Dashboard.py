...
#     fig = px.bar(churn_counts, x='churn', y='count', title='Churn Distribution')
...
# # Internet Service
# with col1:
#     internet_churn_counts = data.groupby(['internetservice', 'churn']).size().reset_index(name='count')
#     fig = px.bar(internet_churn_counts, x='internetservice', y='count', color='churn', barmode='group', title='Internet Service Usage by Churn')
#     fig.update_layout(uniformtext_minsize=8, uniformtext_mode='hide')
#     st.plotly_chart(fig, use_container_width=True)

# # Phone Service
# with col2:
#     phone_churn_counts = data.groupby(['phoneservice', 'churn']).size().reset_index(name='count')
#     fig = px.bar(phone_churn_counts, x='phoneservice', y='count', color='churn', barmode='group', title='Phone Service Usage by Churn')
#     fig.update_layout(uniformtext_minsize=8, uniformtext_mode='hide')
#     st.plotly_chart(fig, use_container_width=True)
import streamlit as st
import pandas as pd
